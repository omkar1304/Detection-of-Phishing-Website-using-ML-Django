...
def generate_data_set(url):
    ...
    if dns == 1:
        ...
        if expiration_date is None:
            ...
            data_set.append(
                0)  # If it is a type of list then we can't select a single value from list. So,it is regarded as suspected website
        else:
...
def noa(url):
    urls = pd.read_csv('app/dataset.csv')
    urls = urls.drop('index', axis=1)
    x = urls.drop('Result', axis=1)
    y = urls['Result']
    ...
    x_test_new = generate_data_set(url)
